...
- (void) performSelect{
    ...
    if(self.fromMode == fromPickup)
        [DataStore instance].addressInfoForPickup = self.selAddressInfo;
    else
        [DataStore instance].addressInfoForDropOff = self.selAddressInfo;
    
    [self dismissViewControllerAnimated:YES completion:^{
        NSString *strNotification = @"";
        
        if(self.fromMode == fromPickup)  strNotification = N_ContactSelectedForPickup;
        if(self.fromMode == fromDropOff) strNotification = N_ContactSelectedForDropOff;
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if(buttonIndex == 0) [self performSelect];
    if(buttonIndex == 1) [self performEdit];
    if(buttonIndex == 2) [self performRemove];
}
...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSMutableArray *shareList =[[NSMutableArray alloc] initWithObjects:LocalizedString(@"pop_menu_select"), LocalizedString(@"pop_menu_edit"), LocalizedString(@"pop_menu_remove"), nil];
